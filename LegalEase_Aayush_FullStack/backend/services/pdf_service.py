...
def build_cover(story,sty,data,theme):
    ...
    title_block=Table([
        [Paragraph(R(data.get("report_title") or "LegalEase Analysis Report"), sty["brand"])],
        [Spacer(1,2)],
    ...
    idea_raw = (data.get("idea") or "").strip()
    idea_block = None
    if idea_raw:
        idea_text = idea_raw if len(idea_raw) <= 800 else idea_raw[:797] + "..."
        idea_block = Table([
            [Paragraph("<b>Your input (idea)</b>", sty["lbl_a"]), Paragraph(R(idea_text), sty["body"])],
        ], colWidths=[72, CW - 72])
        idea_block.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), INK_SOFT),
    ...
    if idea_block is not None:
        ...
        story.append(idea_block)
    story.append(meta)
    story.append(Spacer(1,12))

# ── Suggestions (follow-up questions from AI) ─────────────────────────────────
def build_suggestions(story, sty, items, theme):
    if not items:
        return
    sec_hdr(story, sty, "Suggestions", theme)
    for q in items:
        t = str(q).strip()
        if t:
            story.append(Paragraph(R(f"• {t}"), sty["body"]))
...
def generate_pdf(data: dict, output_path: str) -> str:
    ...
    doc=SimpleDocTemplate(output_path,pagesize=A4,
        ...
        title=data.get("report_title") or f"LegalEase AI - {data.get('business_name','Report')}",
    ...
    build_suggestions(story, sty, data.get("follow_up_questions", []), theme)
